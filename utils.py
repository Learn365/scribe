...
class DataLoader:
    ...
    def preprocess(self, stroke_dir, ascii_dir, data_file):
        ...
        # function to read each individual xml file
        def get_ascii(filename, line_number):
        ...
        for i in range(len(file_list)):
            if file_list[i][-3:] == 'xml':
                stroke_file = file_list[i]
                # print 'processing '+stroke_file
                stroke = convert_stroke_to_array(get_strokes(stroke_file))

                ascii_file = stroke_file.replace("lineStrokes", "ascii")[:-7] + ".txt"
                line_number = stroke_file[-6:-4]
                line_number = int(line_number) - 1
                ascii = get_ascii(ascii_file, line_number)
                if len(ascii) > 10:
